...
func (ctrl *glusterfsController) shouldDelete(volume *v1.PersistentVolume) bool {
	...
	path := fmt.Sprintf("glusterfs-volume-%s", volume.ObjectMeta.Name)
...
func (ctrl *glusterfsController) provision(options VolumeOptions) (*v1.PersistentVolume, error) {
	endpoint, path, err := ctrl.createVolume(options.PVName)
	if err != nil {
		return nil, err
	}
	glog.V(1).Infof("endpoint and path returned :%v %v", endpoint, path)
